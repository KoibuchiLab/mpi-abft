...
#include <zlib.h>
#include <stdint.h>
#include "abft_optweb.h"
#include "Lib_xbar/Xbar_SR.h"

//abft: crc --> hamming
void MPI_Bcast_abft_optweb(unsigned int *buffer, int count, int root, int rank, int procs, int optweb_on_off)
{
  uint32_t crc = 0;
  ...
  int data_bytes = count*sizeof(int);
  unsigned char* data_bits = (unsigned char*) malloc(sizeof(unsigned char)*data_bytes);
  unsigned int* data_bits_ui = (unsigned int*) malloc(sizeof(unsigned int)*count);

  if(rank == root)
  {
      memcpy(data_bits, buffer, data_bytes);
      memcpy(data_bits_ui, buffer, data_bytes);
  ...
  {
    OPTWEB_MPI_Bcast(data_bits_ui, count, MPI_INT, root, MPI_COMM_WORLD);
    if(rank != root)
    {
      memcpy(data_bits, data_bits_ui, data_bytes);
    }    
  }
...
int block_size(int data_bytes)
{
  ...
  int bits = 1/ber; //10000000
  int bytes = bits/8; //1250000
  int bs = data_bytes; 
  if(bs > bytes)
  {
    bs = bytes;
