...
int main(int argc, char *argv[])
{
	...
	for(jj=0; jj < NODE_MAX; jj++)
		for(ii=0; ii<data_num; ii++)
			input_data[ii+data_num*jj] =ii+ jj;

	MPI_Barrier( MPI_COMM_WORLD );
	//OPTWEB_MPI_Barrier( );
	printf("Start Alltoall Size:%dByte rank:%d\n", atoi(argv[1]), rank);
	OPTWEB_MPI_Alltoall(input_data, data_num,  MPI_INT
        			,output_data, data_num, MPI_INT, MPI_COMM_WORLD);
	printf("End Alltoall rank:%d\n", rank);
	/* close devices */
	OPTWEB_MPI_Finalize();
	if(input_data)
		free(input_data);
