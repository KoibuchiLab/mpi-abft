...
int main(int argc, char *argv[])
{
	...
	for(ii=0; ii<data_num; ii++)
		input_data[ii] =ii+rank;

	MPI_Barrier( MPI_COMM_WORLD );

	printf("Start Gather Size:%dByte rank:%d\n", atoi(argv[1]), rank);
	OPTWEB_MPI_Gather(input_data, data_num, MPI_INT, output_data, data_num
        			, MPI_INT, recv_rank, MPI_COMM_WORLD);
	 printf("End Gather rank:%d\n", rank);
