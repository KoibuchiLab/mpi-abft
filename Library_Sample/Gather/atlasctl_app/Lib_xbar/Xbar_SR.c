...
int OPTWEB_MPI_Send(unsigned int input_data[], int data_num, MPI_Datatype sendtype, int  to_send_rank, int tag, MPI_Comm comm)
{
	...
	unsigned int node_connect, to_send_node;
        unsigned int barrier_port;

        barrier_port = 0xFFFF0001;
        //barrier_port -= ((to_send_rank+1) <<16);
        //barrier_port -= ((my_rank+1) <<16);
        if (to_send_rank==0x0){
          barrier_port -= 0x00010000;
        }else if (to_send_rank==0x1){
          barrier_port -= 0x00020000;
        }else if (to_send_rank==0x2){
          barrier_port -= 0x00040000;
        }else if (to_send_rank==0x3){
          barrier_port -= 0x00080000;
        }else if (to_send_rank==0x4){
          barrier_port -= 0x00100000;
        }else if (to_send_rank==0x5){
          barrier_port -= 0x00200000;
        }else if (to_send_rank==0x6){
          barrier_port -= 0x00400000;
        }else if (to_send_rank==0x7){
          barrier_port -= 0x00800000;
        }
        if (my_rank==0x0){
          barrier_port -= 0x00010000;
        }else if (my_rank==0x1){
	...
	//Send
        offset = SG_OFFSET_SG_BASE_0 + OFFSET_SG_NODE_CONNECT;
        data = node_connect;
        ccb_mem_write(atctrl, bar, offset, data);

        offset = SG_OFFSET_SG_BASE_0 + OFFSET_SG_BARRIER;
        data = barrier_port;
        ccb_mem_write(atctrl, bar, offset, data);

        offset = SG_OFFSET_SG_BASE_0 + OFFSET_SG_END;
        data = VALUE_SG_END_00;
        ccb_mem_write(atctrl, bar, offset, data);


        offset = SG_OFFSET_SG_BASE_1 + OFFSET_SG_NODE_CONNECT;
        data = node_connect;
        ccb_mem_write(atctrl, bar, offset, data);

        offset = SG_OFFSET_SG_BASE_1 + OFFSET_SG_BARRIER;
        data = barrier_port;
        ccb_mem_write(atctrl, bar, offset, data);

        offset = SG_OFFSET_SG_BASE_1 + OFFSET_SG_END;
        data = VALUE_SG_END_00;
        ccb_mem_write(atctrl, bar, offset, data);
			...
			, int tag,  MPI_Comm comm, MPI_Status* status)
{
	...
	unsigned int recv_port_val;
        ...
        barrier_port = 0xFFFF0001;
        if (from_send_rank==0x0){
          barrier_port -= 0x00010000;
        }else if (from_send_rank==0x1){
          barrier_port -= 0x00020000;
        }else if (from_send_rank==0x2){
          barrier_port -= 0x00040000;
        }else if (from_send_rank==0x3){
          barrier_port -= 0x00080000;
        }else if (from_send_rank==0x4){
          barrier_port -= 0x00100000;
        }else if (from_send_rank==0x5){
          barrier_port -= 0x00200000;
        }else if (from_send_rank==0x6){
          barrier_port -= 0x00400000;
        }else if (from_send_rank==0x7){
          barrier_port -= 0x00800000;
        }
        if (my_rank==0x0){
          barrier_port -= 0x00010000;
        ...
        //barrier_port -= ((from_send_rank+1) <<16);
        //barrier_port -= ((my_rank+1) <<16);
			...
			, int source, int tag, MPI_Comm comm, MPI_Status * status)
{
	...
	unsigned int node_connect, recv_port_val;
        ...
        if (target_rank==0x0){
          barrier_port -= 0x00010000;
        }else if (target_rank==0x1){
          barrier_port -= 0x00020000;
        }else if (target_rank==0x2){
          barrier_port -= 0x00040000;
        }else if (target_rank==0x3){
          barrier_port -= 0x00080000;
        }else if (target_rank==0x4){
          barrier_port -= 0x00100000;
        }else if (target_rank==0x5){
          barrier_port -= 0x00200000;
        }else if (target_rank==0x6){
          barrier_port -= 0x00400000;
        }else if (target_rank==0x7){
          barrier_port -= 0x00800000;
        }
        if (my_rank==0x0){
          barrier_port -= 0x00010000;
        ...
        //barrier_port -= ((target_rank+1) <<16);
        //barrier_port -= ((my_rank+1) <<16);
        //printf("target_rank=0x%x\n",target_rank);
        //printf("my_rank=0x%x\n",my_rank);
